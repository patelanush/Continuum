...
## Observing a coding run

With `make observability-up`, `make trace-demo` runs the deterministic fixture, approves it, and verifies one Tempo trace contains API/Kafka scheduling, execution, agent turns, model/tool calls, patch/test sandbox commands, approval, and Git commit. The committed JSON summary contains only identifiers, counts, status, and the local commit SHA. `uv run python scripts/trace_workflow.py WORKFLOW_ID` locates any local trace. Prompts, source, patches, tool results, and command output are excluded from telemetry; see [privacy policy](OBSERVABILITY_PRIVACY.md).
