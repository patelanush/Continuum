...
Use `make faultlab-coding-smoke` or `uv run continuum-faultlab campaign coding-smoke --seed 42`. The clean-code-commit `fffc5a5` experiment `7edc7c61-b36f-4219-9f7b-42fd4ea22d21` recorded **11/11 correct trials** across eleven scenarios: eight injected faults, seven recovery-required trials recovered, zero duplicate state transitions, and no duplicate local commits or patches observed. Ten trials created workflows: eight succeeded and two deliberately failed closed (divergence and traversal); the command-timeout trial inspected a sandbox command directly. Three executors participated. Recovery time among seven measured replacements was median **6,032.449 ms**, p95 **16,218.396 ms**; workflow elapsed among ten workflows was median **25,252.505 ms**, p95 **54,185.511 ms**. The campaign ran for **318.578 seconds**. These timings include local Docker contention and are not throughput or production reliability claims. The [generated report](../benchmarks/results/phase6-coding-summary.md) is committed; raw trial JSONL remains under ignored `artifacts/faultlab/7edc7c61-b36f-4219-9f7b-42fd4ea22d21/` on this machine. This cohort remains separate from Phase 4/5 results. The sandbox is local process isolation, not a hostile-code security certification; see [SANDBOX_SECURITY.md](SANDBOX_SECURITY.md).
